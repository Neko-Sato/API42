...
C_HEADER_PATTERN = re.compile(r'^\s*/\*.*\*/\s*$')
SCRIPT_HEADER_PATTERN = re.compile(r'^\s*#.*#\s*$')

TARGET_EXTENSIONS = {'.c', '.h', '.cpp', '.hpp'}
TARGET_FILES = {'Makefile', 'makefile', 'README.md'}


def has_valid_header(file: str, line: str) -> bool:
    if file in {'Makefile', 'makefile', 'README.md'}:
        return SCRIPT_HEADER_PATTERN.match(line) is not None
    return C_HEADER_PATTERN.match(line) is not None
...
def checker(file_path: str, allowed: set[str]):
    try:
        ...
        if not header_lines or not has_valid_header(os.path.basename(file_path), header_lines[0]):
    ...
    for path in pathes:
        if os.path.isfile(path):
            ...
            for root, _, files in os.walk(path):
                for file in files:
                    if os.path.splitext(file)[1] in TARGET_EXTENSIONS or file in TARGET_FILES:
